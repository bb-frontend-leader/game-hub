...
export class GameMain extends Scene {
  ...
  create() {
    ...
    this.combat = createCombat({
      ...
      // ✅ FALTABA ESTO
      setHeroDisplayPct: (pct) => {
        this.heroDisplayPct = pct;
      },
      getHeroDisplayPct: () => this.heroDisplayPct,

      setPlantDisplayPct: (pct) => {
        this.plantDisplayPct = pct;
      },
  ...
  createQuestionPanel() {
    ...
    this.questionDom = this.add.dom(x, y).setOrigin(0, 0).setDepth(999).createFromHTML(`
  ...
  renderAnswerCards(options: Option[]) {
    ...
    options.forEach((opt, i) => {
      const card = new Card(this, startX + i * spacing, cardY, {
        id: opt.id,
        text: opt.text,
        correct: opt.correct,
  ...
  private drawHpBar(
    ...
    g.fillStyle(color, 0.95).fillRoundedRect(
      innerX,
      innerY,
      innerW,
      innerH,
  ...
  private curtainDownDom(duration = 380) {
    return new Promise<void>((resolve) => {
      ...
      this.curtainDomEl.addEventListener(
        "transitionend",
        () => {
  ...
  private async showHurtOverlay() {
    const life =
      this.hp > 1
        ? "Te quedan <b>" + this.hp + "</b> vidas"
        : " Te queda <b>" + this.hp + "</b> vida";
    await this.showOverlay({
      mode: "hurt",
      title: "¡Auch!",
      subtitle: `${life}.<br/><br/>
